...
import type { BookDoc, FavoriteItem, SortOrder } from "../types";

const uniqueByKey = (arr: BookDoc[]): BookDoc[] => {
...
interface UseFilteredSortedBooksParams {
  books?: BookDoc[];
  favorites?: string[];
  favoritesMeta?: FavoriteItem[];
  search?: string;
  sortOrder: SortOrder;
...
const useFilteredSortedBooks = ({
  ...
  onlyFavorites = false,
  statusFilter = "all",
}: UseFilteredSortedBooksParams): BookDoc[] => {
  return useMemo(() => {
